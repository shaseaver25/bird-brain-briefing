...
-- Helper so each policy/grant block is safe to re-run.
...
-- QuickBooks auth: single-row store for the rotating OAuth refresh token
-- (QB rotates the refresh token on every use). Seeded once from the
-- QUICKBOOKS_REFRESH_TOKEN secret; the function updates it each run.
...
create table if not exists public.guidebooks (
  ...
  -- Concept-dependency knowledge graph: { nodes:[{id,label}], edges:[{from,to}] }
...
begin
  -- clients: admins read/insert/update; else authenticated
