-- MockingJay post lifecycle: drafts persist here with an approval status so
-- the Approve / Request-revision / Schedule buttons do real work, and so the
-- platform scorecard can read REAL post history instead of guessing.
...
-- Admins read; admins may also update status (approve/revise/schedule) and
-- discard from the dashboard. Inserts happen via the service role (the agent).
do $$
begin
  if exists (
