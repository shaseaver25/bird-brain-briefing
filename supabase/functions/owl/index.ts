...
Aim for 8-15 items spanning federal + several states. Be accurate; if uncertain about a specific bill, omit it.`;
      ...
      const summaryPrompt = `You are Eagle (the team's Legal Eagle), a legislative analyst. Topics tracked: ${topics.join(", ")}. Below is a JSON list of bills currently being tracked. Write a concise 3-5 sentence executive summary highlighting key trends, urgent items, and notable conflicts between state vs federal approaches. Base the summary ONLY on the bills listed below — do not reference bills, events, or outcomes that are not in the list. Plain prose, no markdown.
